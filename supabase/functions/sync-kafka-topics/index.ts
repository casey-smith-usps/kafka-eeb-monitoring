      ...
      try {
        ...
        if (schemaRegistryUrl) {
          try {
            ...
            const schemaUrl = `${schemaRegistryUrl}/subjects/${schemaSubject}/versions/latest`;

            const schemaAuthKey = schemaRegistryKey || kafkaApiKey;
            const schemaAuthSecret = schemaRegistrySecret || kafkaApiSecret;
            const schemaAuthHeader = schemaAuthKey && schemaAuthSecret
              ? `Basic ${btoa(`${schemaAuthKey}:${schemaAuthSecret}`)}`
              : undefined;
            ...
            if (schemaResponse.ok) {
              ...
              let schemaDefinition = schemaData.schema;

              if (typeof schemaDefinition === 'string') {
                schemaDefinition = JSON.parse(schemaDefinition);
              }

              topicData.latest_schema = schemaDefinition;
              topicData.schema_version = schemaData.version;
              topicData.schema_registry_url = `${schemaRegistryUrl}/subjects/${schemaSubject}`;
              topicData.schema_last_synced = new Date().toISOString();
              results.schemas_synced++;
              console.log(`Schema v${schemaData.version} synced for ${topic.name}`);
            } else if (schemaResponse.status === 404) {
              console.log(`No schema found for ${topic.name}`);
            } else {
              console.log(`Schema Registry returned ${schemaResponse.status} for ${topic.name}`);
            }
          } catch (schemaError: any) {
            console.log(`Schema fetch failed for ${topic.name}: ${schemaError.message}`);
          }
        }
